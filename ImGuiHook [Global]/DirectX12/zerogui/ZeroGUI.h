...
namespace ZeroGUI
{
	...
	void SetupCanvas(UCanvas* _canvas, UEngine* _engine)
	{
		canvas = _canvas;
		engine = _engine;
	}

	void SetController(APlayerController* cont)
	{
		controller = cont;
	}
	...
	void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		canvas->K2_DrawText(engine->SmallFont, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	void TextCenter(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(engine->SmallFont, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	void Hotkey(char* name, FVector2D size, int* key)
	{
		...
		if (active_hotkey == elements_count)
		{
			...
			TextCenter(txt, textPos, FLinearColor{ 1.0f, 1.0f, 1.0f, 1.0f }, false);
